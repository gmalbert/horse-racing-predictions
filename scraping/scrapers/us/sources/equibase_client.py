...
BLOCK_INDICATORS = ["Pardon Our Interruption", "reeseSupportDataV2", "impervaSpaSupport", "reeseSkipExpirationCheck"]


def _is_bot_challenge(html: str) -> bool:
    return any(ind in html for ind in BLOCK_INDICATORS)


# ---------------------------------------------------------------------------
# Playwright fallback (handles Imperva / JS challenges)
# ---------------------------------------------------------------------------

def fetch_via_playwright(track_code: str, race_date: str, url: str) -> list[RaceEntry]:
    """Use Playwright + stealth to bypass Imperva and load the Equibase entry page."""
    try:
    ...
    logger.info("Playwright fetch for Equibase [%s] %s → %s", track_code, race_date, url)
    final_html = ""
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # Give Imperva challenge extra time to complete and page to reload
            page.wait_for_timeout(5_000)
            final_html = page.content()
        except PWTimeout:
            logger.warning("Playwright timeout for Equibase [%s] — capturing partial content", track_code)
            try:
                final_html = page.content()
            except Exception:
                pass
        except Exception as exc:
            logger.error("Playwright navigation error for %s: %s", track_code, exc)
        finally:
            browser.close()

    if not final_html:
        logger.warning("No HTML captured for %s via Playwright", track_code)
        return []

    if _is_bot_challenge(final_html):
        logger.warning("Imperva still blocking after Playwright for %s — no data", track_code)
        return []

    write_raw(final_html, SOURCE_NAME + "_playwright", track_code.lower(), race_date)
    entries = parse_equibase_entries(final_html, track_code, race_date)
    logger.info("Playwright parsed %d entries for %s", len(entries), track_code)
    return entries
...
def fetch_track(session, track_code: str, race_date: str) -> list[RaceEntry]:
    ...
    Strategy:
    1. Try direct HTTP GET (fast, no overhead).
    2. If Imperva bot-challenge is detected, fall back to Playwright.
    """
    ...
    if _is_bot_challenge(html):
        logger.info("Bot challenge detected for %s — switching to Playwright", track_code)
        return fetch_via_playwright(track_code, race_date, url)

    write_raw(html, SOURCE_NAME, track_code.lower(), race_date)
    entries = parse_equibase_entries(html, track_code, race_date)
