...
def run_tvg(race_date: str) -> tuple[list[RaceEntry], SourceResult]:
    """TVG/FanDuel GraphQL API — primary US source, covers all TVG-listed tracks."""
    from sources.tvg_client import fetch_all_us_tracks

    result = SourceResult(source="tvg")
    t0 = time.time()
    try:
        entries, tracks_attempted, tracks_with_data = fetch_all_us_tracks(race_date)
        result.tracks_attempted = tracks_attempted
        result.tracks_with_data = tracks_with_data
        result.entries_collected = len(entries)
    except Exception as exc:
...
ALL_SOURCES = ["tvg", "equibase", "usta", "cdi", "nyra", "quarter_horse", "tracksite", "playwright"]
