"""
Parse direct track-site pull outputs into canonical race/runner rows for Harness.

Step 3 of the T1-H direct-source pipeline.

Inputs:
- data/raw/us_t1_h/tracksite/<TRACK>_<date>.json
- optional: data/raw/us_t1_h/tracksite/pdf_extracted_<date>.json

Outputs:
- data/processed/us_t1_h_canonical_<date>.csv
- data/processed/us_t1_h_canonical_<date>.json
- optional append: data/processed/us_t1_h_canonical_all.csv
"""
...
TRACKSITE_DIR = PROJECT_ROOT / "data" / "raw" / "us_t1_h" / "tracksite"
PROCESSED_DIR = PROJECT_ROOT / "data" / "processed"

TARGET_TRACKS = ["NTH", "SCD", "RSC", "RUN", "PLN", "CAL"]
...
def parse_date(date_str: str, append: bool = False) -> tuple[Path, Path, int]:
    ...
    csv_out = PROCESSED_DIR / f"us_t1_h_canonical_{date_str}.csv"
    json_out = PROCESSED_DIR / f"us_t1_h_canonical_{date_str}.json"

    df.to_csv(csv_out, index=False)
    json_out.write_text(df.to_json(orient="records", indent=2), encoding="utf-8")

    if append:
        all_csv = PROCESSED_DIR / "us_t1_h_canonical_all.csv"
        if all_csv.exists():
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Parse T1-H track-site pulls into canonical rows")
    parser.add_argument("--date", required=True, help="Date in YYYY-MM-DD")
    parser.add_argument("--append", action="store_true", help="Append into data/processed/us_t1_h_canonical_all.csv")
