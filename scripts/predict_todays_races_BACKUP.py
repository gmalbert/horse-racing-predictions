...
def strip_country_suffix(name):
    """Remove country suffixes like (IRE), (GB), (FR), (USA) from horse names"""
    if pd.isna(name):
        return ''
...
def get_historical_data_path():
    """Get the best available historical data file (prioritize most features with current data)"""
    data_dir = DATA_DIR / "processed"
    
    # Priority: connections_v2 (142 features, current data) > enhanced_form (123 features)
    connections_v2_path = data_dir / 'race_scores_connections_v2.parquet'
    enhanced_form_path = data_dir / 'race_scores_enhanced_form.parquet'
    ...
    if connections_v2_path.exists():
        import pandas as pd
        df_test = pd.read_parquet(connections_v2_path)
        max_date = df_test['date'].max()
        print(f"[OK] Using connections_v2 (142 features, data through {max_date})")
        return connections_v2_path
    elif enhanced_form_path.exists():
        import pandas as pd
        df_test = pd.read_parquet(enhanced_form_path)
        max_date = df_test['date'].max()
        print(f"[OK] Using enhanced_form (123 features, data through {max_date})")
        return enhanced_form_path
    elif or_context_path.exists():
        print(f"[OK] Using or_context (114 features)")
        return or_context_path
    elif legacy_path.exists():
        print(f"[WARNING] Using race_scores.parquet (71 features - run regenerate_all.ps1)")
        return legacy_path
...
def load_models():
    ...
    with warnings.catch_warnings():
        ...
        # If calibrated model is available, use it; otherwise load raw model
        if calibrated_model is not None:
            ...
            print(f"[OK] Loaded XGBoost model: {WIN_MODEL_FILE.name}")
        else:
            with open(WIN_MODEL_FILE, 'rb') as f:
                win_model = pickle.load(f)
            print(f"[OK] Loaded pickled model: {WIN_MODEL_FILE.name}")
...
def build_horse_features_from_racecard(runner, race_info, historical_df, prediction_date=None):
    ...
    # Normalize names by stripping country suffixes for matching
    horse_name_clean = strip_country_suffix(horse_name).lower()
    
    # Create cleaned column if it doesn't exist (cache for performance)
    ...
    # Filter to this horse's history using cleaned names
    horse_history = historical_df[
        historical_df['horse_clean'] == horse_name_clean
    ].copy()
    
    # DEBUG: Log matching results for first 20 horses
    if hasattr(build_horse_features_from_racecard, 'debug_count'):
        build_horse_features_from_racecard.debug_count += 1
    else:
        build_horse_features_from_racecard.debug_count = 1
    
    if build_horse_features_from_racecard.debug_count <= 20:
        print(f"DEBUG: Horse '{horse_name}' -> cleaned: '{horse_name_clean}' -> {len(horse_history)} records")
        if len(horse_history) > 0:
            sample_names = historical_df[historical_df['horse_clean'] == horse_name_clean]['horse'].head(3).tolist()
            print(f"  Matched DB names: {sample_names}")
            # Show key stats from history
            if 'or' in horse_history.columns and len(horse_history) > 0:
                recent_ors = pd.to_numeric(horse_history.head(3)['or'], errors='coerce').dropna().tolist()
                print(f"  Recent ORs: {recent_ors}")
            if 'pos' in horse_history.columns and len(horse_history) > 0:
                recent_pos = horse_history.head(3)['pos'].tolist()
                print(f"  Recent positions: {recent_pos}")
    
    # Calculate jockey stats from ALL historical data (not just this horse)
    jockey_history = historical_df[
        historical_df['jockey'].fillna('').str.lower() == jockey_name.lower()
    ] if jockey_name != 'Unknown' else pd.DataFrame()
    
    # Jockey-course stats
    course = race_info['course']
    jockey_course_history = jockey_history[
        jockey_history['course'].fillna('').str.lower() == course.lower()
    ] if len(jockey_history) > 0 else pd.DataFrame()
    
    # Jockey-trainer stats
    jockey_trainer_history = jockey_history[
        jockey_history['trainer'].fillna('').str.lower() == trainer_name.lower()
    ] if len(jockey_history) > 0 and trainer_name != 'Unknown' else pd.DataFrame()
    
    if horse_history.empty:
        # New horse or no data - use OR-based estimates instead of all zeros
        # This provides better discrimination when historical data is unavailable
        or_val = float(runner.get('ofr') or 0) if runner.get('ofr') and runner.get('ofr') != '-' else 0
        
        # Get age for additional context
        ...
        # Get days since last run for recency factor
        days_since = extract_days_since_last_run(runner.get('last_run'))
        recency_factor = 1.0
        if days_since < 14:
            recency_factor = 1.2  # Recent run bonus
        elif days_since > 120:
            recency_factor = 0.8  # Long absence penalty
        
        # Estimate career stats from OR (Official Rating) WITH MORE VARIATION
        # OR typically ranges from 60-180, with 100 being average
        # Higher OR suggests better past performance
        if or_val > 0:
            # MORE AGGRESSIVE win rate estimation to create variation
            # OR 140+ → 25%, OR 130 → 20%, OR 120 → 15%, OR 110 → 11%, OR 100 → 8%, OR 80 → 4%
            base_win_rate = max(0.02, min(0.30, (or_val - 70) / 280))
            
            # Adjust by recency
            estimated_win_rate = base_win_rate * recency_factor
            estimated_place_rate = min(0.65, estimated_win_rate * 3.0)
            
            # Vary career runs by age and OR
            # Higher OR horses likely have more runs
            or_factor = max(0.8, min(1.5, or_val / 120))
            estimated_runs = max(3, int((age_num - 2) * 6 * or_factor))
            
            # Earnings scale with OR and runs
            estimated_earnings = estimated_win_rate * estimated_runs * 8000 * or_factor
        else:
            # No OR available - use minimal estimates with age-based variation
            estimated_win_rate = 0.05 + (age_num / 200)  # Slight age variation
            estimated_place_rate = 0.15 + (age_num / 100)
            estimated_runs = max(3, (age_num - 2) * 4)
            estimated_earnings = 500 * estimated_runs
        
        features = {
            'career_runs': estimated_runs,
            'career_win_rate': estimated_win_rate,
            'career_place_rate': estimated_place_rate,
            'career_earnings': estimated_earnings,
            'cd_runs': max(1, int(estimated_runs * 0.2)),  # Assume 20% at this course/distance
            'cd_win_rate': estimated_win_rate * 0.85,  # Slightly lower C/D record
            'class_num': extract_class_num(race_info.get('race_class', 'Class 4')),
            'class_step': 0,
            'or_numeric': or_val,
            'or_change': 0,
            'or_trend_3': 0,
            # Better position for higher OR horses
            'avg_last_3_pos': max(2.5, min(12, 13.5 - (or_val / 15))),
            'wins_last_3': 1 if estimated_win_rate > 0.14 else 0,
            'days_since_last': days_since,
            'field_size': int(race_info.get('field_size') or 10),
        ...
        try:
            ...
            age_num = 4
        features['age'] = age_num
        features['is_peak_age'] = 1 if 5 <= age_num <= 7 else 0
        features['is_3yo'] = 1 if age_num == 3 else 0
        features['is_veteran'] = 1 if age_num >= 9 else 0
        features['age_vs_avg'] = 0  # Calculated in second pass
        
        # Distance features
        dist_f = extract_distance_furlongs(race_info.get('distance_f', 8))
        features['is_sprint'] = 1 if dist_f <= 7 else 0
        features['is_mile'] = 1 if 7 < dist_f <= 10 else 0
        features['is_middle'] = 1 if 10 < dist_f <= 14 else 0
        features['is_staying'] = 1 if dist_f > 14 else 0
        
        # Race type features (inferred from race name)
        race_name = race_info.get('race_name', '').lower()
        features['is_handicap'] = 1 if 'handicap' in race_name else 0
        features['is_maiden'] = 1 if 'maiden' in race_name else 0
        features['is_pattern'] = 1 if any(x in race_name for x in ['grade', 'group', 'listed']) else 0
        
        # Prize (if available)
        prize_val = race_info.get('prize', 0)
        try:
            prize_num = float(str(prize_val).replace('£', '').replace(',', ''))
            features['prize_log'] = np.log1p(prize_num)
        except:
            features['prize_log'] = 8.5  # Default for typical Class 2-4 race
        
        # Form/gear features (can't infer from racecard)
        features['avg_btn_last_3'] = 5.0  # Default
        features['unlucky_last'] = 0
        features['has_blinkers'] = 0
        features['has_visor'] = 0
        features['first_time_blinkers'] = 0
        features['gear_changed'] = 0
        
        # Sire features (can't populate without pedigree data)
        features['sire_win_rate_v2'] = 0.10  # Assume average sire
        features['sire_place_rate_v2'] = 0.30
        features['sire_turf_win_rate'] = 0.10 if features['is_turf'] else 0.05
        features['sire_distance_win_rate'] = max(0.05, estimated_win_rate * 0.8)  # Use OR-based estimate
        features['sire_going_win_rate'] = max(0.05, estimated_win_rate * 0.8)
        
        # Going/speed features (defaults)
        features['going_pref_score'] = 0.5  # Neutral
        features['recent_speed_fig'] = max(60, or_val * 0.5) if or_val > 0 else 60
        features['speed_vs_par'] = 0
        
        # Pace features (will be calculated in second pass)
        features['pace_style_leader'] = 0
        features['pace_style_closer'] = 0
        features['race_leader_count'] = 0
        features['race_closer_count'] = 0
        features['style_advantage'] = 0
        
        # Connection features (can't populate without historical data)
        features['trainer_course_win_rate'] = 0.10  # Assume average
        features['jockey_horse_combo_runs'] = 0
        features['jockey_horse_combo_win_rate'] = 0.0
        features['trainer_jockey_combo_runs'] = 0
        features['trainer_jockey_combo_win_rate'] = 0.10
    ...
    horse_history['pos'] = pd.to_numeric(horse_history['pos'], errors='coerce')
    horse_history['dist_f'] = pd.to_numeric(horse_history['dist_f'], errors='coerce')
    ...
    cd_history = horse_history[
        (horse_history['course'].fillna('').str.lower() == course.lower()) &
        (horse_history['dist_f'].between(dist_f - 1, dist_f + 1))
    ]
    cd_runs = len(cd_history)
    # Ensure pos is numeric
    if cd_runs > 0:
        cd_history['pos'] = pd.to_numeric(cd_history['pos'], errors='coerce')
        cd_wins = (cd_history['pos'] == 1).sum()
    else:
        cd_wins = 0
    ...
    if 'or' in horse_history.columns:
        recent_or = pd.to_numeric(horse_history.head(3)['or'], errors='coerce').dropna()
    else:
        recent_or = pd.Series([or_numeric])
    ...
    # Race context
    field_size = int(race_info.get('field_size') or len(race_info.get('runners', [])) or 10)
    is_turf = 1 if race_info.get('surface') == 'Turf' else 0
    going_numeric = encode_going(race_info.get('going', 'Good'))
    ...
    if len(jockey_history) > 0:
        jockey_history['pos'] = pd.to_numeric(jockey_history['pos'], errors='coerce')
        jockey_career_wins = (jockey_history['pos'] == 1).sum()
    else:
        jockey_career_wins = 0
    
    jockey_course_runs = len(jockey_course_history)
    if len(jockey_course_history) > 0:
        jockey_course_history['pos'] = pd.to_numeric(jockey_course_history['pos'], errors='coerce')
        jockey_course_wins = (jockey_course_history['pos'] == 1).sum()
    else:
        jockey_course_wins = 0
    
    jockey_trainer_runs = len(jockey_trainer_history)
    if len(jockey_trainer_history) > 0:
        jockey_trainer_history['pos'] = pd.to_numeric(jockey_trainer_history['pos'], errors='coerce')
        jockey_trainer_wins = (jockey_trainer_history['pos'] == 1).sum()
    else:
    ...
    prev_headgear = horse_history.head(1)['headgear'].iloc[0] if len(horse_history) > 0 and 'headgear' in horse_history.columns else ''
    prev_headgear = str(prev_headgear or '').lower()
    features['first_time_blinkers'] = 1 if (features['has_blinkers'] == 1 and 'b' not in prev_headgear) else 0
    features['gear_changed'] = 1 if (headgear != prev_headgear) else 0
...
def predict_race(racecard, historical_df, win_model, place_model, show_model, feature_cols, prediction_date=None):
    ...
    for runner in racecard.get('runners', []):
        ...
        # DEBUG: Show first 5 horses' key features
        if horse_debug_count < 5:
            horse_name = runner.get('horse') or runner.get('name', 'Unknown')
            print(f"  FEATURES for '{horse_name}':")
            print(f"    career_runs={features.get('career_runs')}, career_win_rate={features.get('career_win_rate'):.3f}")
            print(f"    or_numeric={features.get('or_numeric')}, avg_last_3_pos={features.get('avg_last_3_pos'):.2f}")
            print(f"    jockey_career_win_rate={features.get('jockey_career_win_rate'):.3f}")
            horse_debug_count += 1
